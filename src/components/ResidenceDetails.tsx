...
// The room facts, pricing and specs — shared verbatim between the homepage
// Residences section (ResidencesSection.tsx, scroll-animated) and the
// standalone /residence page (ResidencePage.tsx, page-load animated). One
// content source so the numbers and copy can't drift between the two.
// data-decision-summary / data-decision-item / data-pet-fee are asserted by
// tools/test-landing-critical.mjs against the homepage render — keep them.
const ResidenceDetails: React.FC = () => {
  ...
  useGSAP(
    () => {
      ...
      const cards = rootRef.current.querySelectorAll('.res-card');
      gsap.from(cards, {
        y: 30,
        opacity: 0,
        duration: 0.7,
      ...
      const essentialsCard = rootRef.current.querySelector('.essentials-card');
      ...
      const features = rootRef.current.querySelectorAll('.ess-item');
      gsap.from(features, {
        y: 15,
        opacity: 0,
        duration: 0.5,
